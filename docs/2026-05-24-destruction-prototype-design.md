...
**Constraint 门（v1 关键设计补充，2026-05-26）**：

`Block._integrate_forces` 在 first-contact gate 之后增加第二道门：若 self 和 other 共享任何 Constraint（Weld / Flex），直接跳过冲击伤害。`_has_shared_constraint(other)` 遍历 `connected_constraints` 检查 block_a/block_b。

**目的**：PGS 迭代求解器在受力剧烈时无法完美收敛，PinJoint 连接的 Block 之间必然产生亚像素级分离。后续 substep 位置修正"拉回来" → collision detection 看到新接触 → first-contact gate 放行（上一帧已分离）。这种 solver-induced re-contact 是物理引擎的固有 trade-off，不应产生伤害。

实测验证（本会话）：
- 对角线 Constraint（`threshold = 1.42`，见 §4.6）+ Constraint 门双管齐下后，结构邻居之间的 re-contact 伤害完全消除
- 所有过闸的 contact 均为无约束关系的 debris / 弹丸等异物碰撞
- 从左侧打击 house，右侧不再有异常伤害集中在右上角
...
- **判定基准**：两 Block 中心点欧氏距离 ≤ `block_size × 1.42  # Covers direct (25 px) + diagonal (25√2 ≈ 35.4 px) neighbors; excludes two-step (50 px)` 视为邻居
- **几何后果**：该阈值覆盖直连邻居和对角邻居。每个 Block 最多 8 个邻居（上下左右 + 四角），结构更刚性、更接近整体石造建筑直觉
- **实现**：每个邻居对 = 1 根 PinJoint2D，对角 PinJoint 锚点在两 Block 中心连线的中点；Weld kind 额外把两端 Block.lock_rotation 拨成 true（详见 §4.2）
...
- 两 cube 中心点欧氏距离 ≤ `block_size × 1.42  # Covers direct (25 px) + diagonal (25√2 ≈ 35.4 px) neighbors; excludes two-step (50 px)` 视为邻居
- 覆盖直连和对角邻居，排除两倍距离
- 每对邻居最多 8 个（上下左右 + 四角）
