...
- **Debug 半径可视化**：每个 RadialDamage / RadialBlast 在触发瞬间自绘自己的 `radius` 环（伤害红、爆风橙），与 Effect 视觉占位独立。当前 `Effect.visual_radius_px` 是 VFX 占位（未来替换为爆炸 sprite/粒子），不反映物理范围；调参需要"所见即所得"的物理 debug 通道。统一 `debug_draw_radii: bool` 开关挂 Effect 顶层一键开关
