...
static func create(
	...
	var block := BlockKlass.new()
	block.global_position = pos
	block.initial_health = initial_health
	block.pipeline = pipeline
	# Body 参数（spec §4.1）
	block.freeze = false
	block.freeze_mode = RigidBody2D.FREEZE_MODE_STATIC  # freeze_mode 仅在 freeze=true 时生效
	block.mass = 0.1 

	var phys_mat := PhysicsMaterial.new()
	phys_mat.friction = 0.6
	phys_mat.bounce = 0.05
	block.physics_material_override = phys_mat
	block.linear_damp = 0.05
	block.angular_damp = 0.1
	block.contact_monitor = true
	block.max_contacts_reported = 8  # ImpactWatcher 需要
	...
	block.collision_layer = 4   # layer 4 = block
	block.collision_mask = 4 | 1  # block + world（layer 1 = world）
	block.impact_watcher = impact
	return block
