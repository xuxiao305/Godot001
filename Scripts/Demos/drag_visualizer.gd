...
# # 不加 @tool：弹簧/拖尾只有运行时有意义，而 @tool 会让 _physics_process 在编辑器里
# # 调用非-@tool 的父 DemoLevel.get_drag_body()，触发 "placeholder instance" 报错。
...
# 	# 兼容编辑器残留 @tool 缓存：若 _demo 被加载成 placeholder 实例（基类降级为 Node2D），
# 	# has_method() 返回 false，直接跳过避免 "Attempt to call a method on a placeholder instance" 刷屏。
# 	if _demo == null or not _demo.has_method(&"get_drag_body"):
# 		return
